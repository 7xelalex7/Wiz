...
# PW_Sheet <- gs_title("PW_Data")

# Pulling the data from a given a sheet - Need to define the range
# PW_Data_gs <- gs_read(PW_Sheet, ws = "Sales_Data", range = cell_limits(c(2, 1), c(NA, 15)), col_names = TRUE)
...
#PW_Calendar_gs <- gs_read(PW_Sheet, ws = "PW_Calendar", range = cell_limits(c(1, 1), c(NA, 4)), col_names = TRUE)

# Converting data into data format
# PW_Calendar = PW_Calendar_gs %>%
...
setwd('C:/Users/akhelifa/Desktop/PW_Wachi')
# Importing local CSV File
PW_Calendar = read.csv('Date_PW.csv', stringsAsFactors = FALSE) %>%
    mutate(Date = as.Date(Date, format = "%m/%d/%Y")) %>%
    mutate(Fiscal_Month_Start = as.Date(Fiscal_Month_Start, format = "%m/%d/%Y")) %>%
    mutate(Fiscal_Week_Start = as.Date(Fiscal_Week_Start, format = "%m/%d/%Y") )

PW_Data_gs = read.csv('Data_PW_Dummy.csv', stringsAsFactors = FALSE) %>%
    mutate(Date = as.Date(Date, format = "%m/%d/%Y"))
```
...
renderHighchart({
    hchart(PW_Week, "bar", hcaes(x = Cum_500, y = Time_Period, names = as.character(Count)), pointWidth = 10) %>%
...
### Top 12 Lines
```{r}
# Display the top 12 Lines and thei Weekly Run Rate
PW_Sales_All = PW_Data %>%
    ...
    left_join(PW_First_Week, by = c("Collection", "Line")) %>%
    ...
    select(Collection, Line, Count, Number_Weeks, Weekly_RR)

renderHighchart({
    hchart(head(PW_Sales_All, 12), "bar", hcaes(x = Line, y = Weekly_RR, group = Collection), pointWidth = 10) %>%
    hc_add_theme(hc_theme_economist())
    })
```
...
tm <- treemap(PW_Sales_All, index = c("Collection", "Line"),
...
PW_Eyewear_Gender = PW_Eyewear %>%
    left_join(PW_Gender, by = c("Collection", "Line", "Total")) %>%
    mutate(Males_Perc = Males_Perc * 100) %>%
    mutate(Solar_Perc = Solar_Perc * 100)

renderHighchart({
highchart() %>% 
    hc_add_series_scatter(PW_Eyewear_Gender$Males_Perc, PW_Eyewear_Gender$Solar_Perc,
                       PW_Eyewear_Gender$Total, PW_Eyewear_Gender$Collection,
                       PW_Eyewear_Gender$Line,
                       dataLabels = list(
                         enabled = TRUE,
                         format = "{point.label}"
                       )) %>% 
...
PW_Sales_Month_Line_All = PW_Sales_Month_Line %>%
...
Start_Date = min(PW_Sales_Month_Line_All$Fiscal_Month_Start)

# Make sure the data is complete - no missing month
PW_Sales_Month_Full = PW_Sales_Month_Line_All %>%
...
Lines_Sales_ts <- ts(PW_Sales_Month_Line_All$Count, start = c(year(Start_Date), month(Start_Date)), frequency = 12)
...
PW_Data_Eyewear_All = PW_Data_Eyewear %>%
    group_by(Eyewear) %>%
    summarize(Count = sum(Count)) %>%
    ungroup() %>%
...
PW_Data_Collection_Reac =  reactive({
PW_Data_Collection_All = PW_Data_Color %>%
    group_by(Collection) %>%
    summarize(Count = sum(Count)) %>%
    ungroup()
    })
...
renderHighchart({
PW_Data_Collection_Reac() %>%
    # PW_Data_Collection_All %>%
...
PW_Data_Age_Reac = reactive({
  # Get the split by Age
PW_Data_Age_All = PW_Data_Age %>%
    group_by(Age) %>%
    summarise(Count = sum(Count)) %>%
    ungroup() %>%
...
PW_Data_Gender_Reac = reactive({
    # Get the split by gender
PW_Data_Gender_All = PW_Data_Gender %>%
    group_by(Gender) %>%
    summarise(Count = sum(Count)) %>%
    ungroup() %>%
    mutate(Perc = round(Count / sum(Count),2)) %>%
...
renderHighchart({highchart() %>% 
    #Data
    hc_add_series(PW_Data_Age_Reac(), "column", hcaes(x = Age, y = Perc), name = "Age") %>%
    hc_add_series(PW_Data_Gender_Reac(), "pie", hcaes(name = Gender, y = Perc), name = "Gender") %>%
    #hc_add_series(PW_Data_Age_All, "column", hcaes(x = Age, y = Perc), name = "Age") %>%
    #hc_add_series(PW_Data_Gender_All, "pie", hcaes(name = Gender, y = Perc), name = "Gender") %>%
    #Options
    hc_plotOptions(
    ...
    hc_xAxis(categories = PW_Data_Age_Reac()$Age) %>%
    #hc_xAxis(categories = PW_Data_Age_All$Age) %>%
    hc_add_theme(hc_theme_economist())
})
```
...
# Logic is very close than the previous Page
 selectInput("Line", label = h3("Select Line"), 
    choices = sort(unique(PW_Data$Line)), 
    selected = "ModelA3")
```
...
Lines_Sales_ts_fit_Filter_Reac = reactive({
    ...
    PW_Sales_Month_Line_Filter = PW_Sales_Month_Line %>%
        filter(Line == input$Line) 
    
    Start_Date = min(PW_Sales_Month_Line_Filter$Fiscal_Month_Start)
    
    PW_Sales_Month_Full = PW_Sales_Month_Line_Filter %>%
            complete(Fiscal_Month_Start = seq.Date(Start_Date, Max_Date, by = "month"),  
                     fill = list(Count = 0)) %>%
        arrange(Fiscal_Month_Start)
    
    Lines_Sales_ts <- ts(PW_Sales_Month_Line_Filter$Count, start = c(year(Start_Date), month(Start_Date)), frequency = 12)
...
```{r}
PW_Data_Eyewear_Filter_Reac =  reactive({
    PW_Data_Eyewear_Filter = PW_Data_Eyewear %>%
        filter(Line == input$Line) %>%
        mutate(Perc = round(Count / sum(Count),2)) %>%
        mutate(Perc = 100 * Perc)
...
### Color Split
```{r}
PW_Data_Color_Filter_Reac =  reactive({
    PW_Data_Color_Filter = PW_Data_Color %>%
        filter(Line == input$Line) %>%
        mutate(Perc = round(Count / sum(Count),2)) %>%
        mutate(Perc = 100 * Perc)
    })
...
renderHighchart({
    PW_Data_Color_Filter_Reac() %>%
        #PW_Data_Color_Filter %>%
...
PW_Data_Age_Filter_Reac = reactive({
PW_Data_Age_Filter = PW_Data_Age %>%
    filter(Line == input$Line) %>%
    mutate(Perc = round(Count / sum(Count),2)) %>%
    mutate(Perc = 100 * Perc)
    })

PW_Data_Gender_Filter_Reac = reactive({
PW_Data_Gender_Filter = PW_Data_Gender %>%
    filter(Line == input$Line) %>%
    mutate(Perc = round(Count / sum(Count),2)) %>%
    mutate(Perc = 100 * Perc)
})
...
renderHighchart({highchart() %>% 
    #Data
    hc_add_series(PW_Data_Age_Filter_Reac(), "column", hcaes(x = Age, y = Perc), name = "Age") %>%
    hc_add_series(PW_Data_Gender_Filter_Reac(), "pie", hcaes(name = Gender, y = Perc), name = "Gender") %>%
    #hc_add_series(PW_Data_Age_Filter, "column", hcaes(x = Age, y = Perc), name = "Age") %>%
    #hc_add_series(PW_Data_Gender_Filter, "pie", hcaes(name = Gender, y = Perc), name = "Gender") %>%
    #Options
    hc_plotOptions(
        series = list(showInLegend = FALSE),
        column = list(colorByPoint = TRUE),
    ...
    hc_yAxis(
        title = list(text = "Split by Age"),
        labels = list(format = "{value}%")) %>%
    hc_xAxis(categories =PW_Data_Age_Filter_Reac()$Age) %>%
    #hc_xAxis(categories = PW_Data_Age_Filter$Age) %>%
    hc_add_theme(hc_theme_economist())
})
```
...
Season_Eyewear = func_get_season_group_month(PW_Sales_Month_Eyewear, "Eyewear", "Count", "Fiscal_Month_Start") %>%
    rename(Eyewear = TYPE)
...
Eyewear_Type = sort(unique(PW_Sales_Month_Eyewear$Eyewear))
...
for (i in Eyewear_Type)
{
  # Filter data for each eyewear
    Eyewear_Data = PW_Sales_Month_Eyewear %>%
        filter(Eyewear == i) %>%
        arrange(Fiscal_Month_Start) %>%
        filter(Fiscal_Month_Start < max(Fiscal_Month_Start)) %>%
